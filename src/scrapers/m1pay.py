...
class PGM1payScraper(BaseScraper):
    
    def __init__(self, account: dict):
        super().__init__(account)
        self.merchant = account.get('merchant', 'Unknown')
        logger.info(f"M1pay scraper for merchant: {self.merchant}")
    
    async def check_if_logged_in(self, page: Page) -> bool:
        try:
            dashboard_element = page.locator('div.dashboard, #main-content')
    ...
    async def fill_login_credentials(self, page: Page):
        logger.info("Filling M1pay login credentials")
        
        email_selector = 'input[name="email"], #email'
    ...
    async def download_files(self, page: Page, download_dir: Path, date_str: str) -> List[Path]:
        logger.info(f"Downloading M1pay reports for {self.merchant} - Date: {date_str}")
        
        downloaded_files = []
        logger.warning("M1pay scraper download_files() not fully implemented")
