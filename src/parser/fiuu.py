...
class FiuuParser:
    ...
    def _parse_fpx(self, file_path: Path, account_label: str) -> List[dict]:
        ...
        for _, row in df.iterrows():
            try:
                tx = {
                    'transaction_id': str(row['Order ID']),
                    'transaction_date': self._parse_date(row['Date']),
                    'amount': float(row['Bill Amt']),
                    'channel': 'FPX',
    ...
    def process_directory(self, directory: Path, account_label: str, run_id: str = None) -> dict:
        ...
        parsed_files = get_parsed_files(account_label, 'fiuu')
        ...
        for file_path in new_files:
            job_id = start_parse_job(file_path.name, account_label, 'fiuu', run_id)
            ...
            try:
                ...
                if transactions:
                    ...
                    result['by_channel'][channel] = result['by_channel'].get(channel, 0) + saved
